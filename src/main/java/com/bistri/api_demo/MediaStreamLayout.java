...
public class MediaStreamLayout extends RelativeLayout implements
		View.OnClickListener, MediaStream.Handler {
...
//		mediaStreams.clear();
	...
	/*
	 * private boolean needResize() { int current_orientation =
	 * this.getResources().getConfiguration().orientation; if ( last_orientation
	 * != current_orientation ) { return true; } if ( last_video_nb !=
	 * mediaStreams.size() ) { return true; } for (MediaStream media :
	 * mediaStreams) { float ratio = media.getVideoRatio(); SurfaceView view =
	 * media.getRender(); float current_ratio = (float)view.getWidth() /
	 * view.getHeight(); if ( (int)(ratio *100) != (int)(current_ratio *100) ) {
	...
	// }
	// int vertMargin = (viewHeight < maxViewHeight) ? (maxViewHeight -
	// viewHeight) / 2 : 0;
	// int horiMargin = (viewWidth < maxViewWidth) ? (maxViewWidth - viewWidth)
	// / 2 : 0;
	...
	public void onClick(View view) {
		...
		for (MediaStream media : mediaStreams) {
			...
			if (view == render_view) {
				Log.d(TAG, (!media.isAudioMute() ? "Mute" : "Unmute")
						+ " peer " + media.getPeerId());
